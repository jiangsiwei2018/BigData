...
#             #temp = np.array(np.multiply(pd.DataFrame(self.A), pd.DataFrame(alpha[:, i-1])))
#             #alpha[:, i] = np.multiply(np.sum(temp, axis=0), self.B.get(self.O[i]))
...
#         total = 0
#         emxi = np.zeros([self.T-1, self.N, self.N])
